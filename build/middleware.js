...
export const middleware = async (request) => {
    ...
    if (!info.success) {
        return new Response(`Ratelimit reached. The maximum amount of requests is currently set to ${requestsLimit} per 6 hours.`, {
            status: 429,
            headers: {
                //Always refers to Requests Per 6h
                "x-ratelimit-limit-requests": "100",
                //Always refers to Requests Per 6h
                "x-ratelimit-remaining-requests": String(requestsLimit - info.remaining),
                //Always refers to Requests Per 6h
                // string in seconds
                "x-ratelimit-reset-requests": String((info.reset - Date.now()) / 1000),
